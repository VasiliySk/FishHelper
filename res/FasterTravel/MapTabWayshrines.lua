...
function MapTabWayshrines:init(control,locations,locationsLookup,recentList,favourites)
	...
	self.Refresh = function(self,nodeIndex,isKeep)
		...
		local faves = GetFavouritesWayshrinesData(favourites,{nodeIndex=nodeIndex1,favourites=favourites, refresh=function(self,control) AddRowToLookup(self,control,favouriteslookup) end}) --Оключен фильтр текущего телепорта
