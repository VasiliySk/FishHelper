...
--[[
    ...
    wayshrinesTab = FasterTravel.MapTabWayshrines(wayshrineControl, _locations, _locationsLookup, recentList1, { list = favouritesList, add = AddFavourite, remove = RemoveFavourite }) -- Отключен блок Recent
	...
	if result then
		...
		df("%s %s expanded to %s", _prefix, f.bold(destination), f.bold(name))
	    end
